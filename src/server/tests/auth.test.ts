...
describe('authenticateAdmin', () => {
  ...
  it('returns null when user does not exist', async () => {
    ...
    const result = await authenticateAdmin('nobody@alcocars.es', 'pass123');
  ...
  it('returns null when password is incorrect', async () => {
    ...
    const result = await authenticateAdmin('admin@alcocars.es', 'wrongpass');
  ...
  it('returns user data without passwordHash when credentials are valid', async () => {
    ...
    const result = await authenticateAdmin('admin@alcocars.es', 'admin123');
