...
import { parseReservaParams } from '../lib/reservaQuery';
...
export default function CheckoutPage() {
  ...
  // Estado inicial desde la URL: /reserva?gama=…&recogida=…&desde=…&hasta=…
  // (compartible y estable ante recargas, a diferencia del state del router).
  const bookingState = useMemo<CheckoutState | null>(() => {
    const params = parseReservaParams(new URLSearchParams(searchParams?.toString() ?? ''));
    if (!params.location && !params.tariffId && !params.category && !params.from) {
      return null;
    }
    return {
      location: params.location,
      tariffId: params.tariffId,
      rentalCategory: params.category,
      dateRange: params.from
        ? { from: `${params.from}T00:00:00`, to: params.to ? `${params.to}T00:00:00` : undefined }
        : undefined,
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps -- solo la primera lectura importa
  }, []);
                  ...
                  <section className={styles.contactPanel}>
                    ...
                    <label className={styles.consentRow}>
                      ...
                      <span>
                        ...
                        <Link href="/legal/politica-privacidad" target="_blank" rel="noreferrer">
